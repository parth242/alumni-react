...
import { MessagePopupDataType } from "utils/datatypes";
...
type RequestType = {
	isMessageConfirm: boolean;
	setIsMessageConfirm: (fl: boolean) => void;
	isMessageCancelled?: boolean;
	setIsMessageCancelled?: (fl: boolean) => void;
	data: MessagePopupDataType;
...
const MessagePopup: React.FC<RequestType> = ({
	isMessageConfirm,
	setIsMessageConfirm,
...
const [formData, setFormData] = useState({
  ...
  // Assuming you have a state to hold the validation errors
			...
			<Transition.Root show={isMessageConfirm} as={Fragment}>
				<Dialog
					...
					onClose={() => {
						...
						if (setIsMessageCancelled) setIsMessageCancelled(true);
					}}>
					<Transition.Child
						as={Fragment}
					...
					<form action="#" onSubmit={MessageModal}>
					<div
						className={`${
							isDark ? "dark" : ""
									...
									<div className="mt-6 mb-4 flex flex-col-reverse gap-2 px-6 text-right sm:flex-row sm:justify-end sm:gap-0">
										...
										<button
											...
											onClick={() => {
												...
												setIsMessageConfirm(false);
												if (setIsMessageCancelled) setIsMessageCancelled(false);
											}}>
