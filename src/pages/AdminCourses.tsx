...
import { useCourses,deleteCourse,statusCourse } from "api/services/courseService";
import { ErrorToastMessage, SuccessToastMessage } from "api/services/user";
import { useMutation } from "react-query";
import { HTTPError } from "ky";
import PaginationBar from "components/ui/paginationBar";
import TableHead from "components/ui/table/tableHead";
import { ICourse, TableHeadType,ConfirmPopupDataType } from "utils/datatypes";
...
function AdminCourses() {
	...
	const ConfirmPopupData : ConfirmPopupDataType =
		{ title: "Course Delete", text: "Are you sure you want to delete Course?" };

	const {
		isLoading,
		data: courseList,
		refetch: fetchCourseList,
		...
		const fetchData = async () => {
		  ...
		  const checkIfIncludes = (value: number) => {
			...
			return rolepermid.includes(value);
			  ...
			  .get(
				  `api/v1/submenu/action=Add&module_alias=courselist`
			  );
			...
			const editmodule = await apiClient
			  .get(
				  `api/v1/submenu/action=Edit&module_alias=courselist`
			  );
			  ...
			  .get(
				  `api/v1/submenu/action=Delete&module_alias=courselist`
			  );
	...
	const { mutate: deleteItem, isLoading: uploadIsLoading } = useMutation(deleteCourse, {
		onSuccess: async () => {
			SuccessToastMessage({
				title: "Delete Course Successfully",
				id: "delete_course_success",
			});
			fetchCourseList();
		},
		onError: async (e: HTTPError) => {
			...
			ErrorToastMessage({ error: e, id: "delete_course" });
		},
	});
	...
	const { mutate: statusItem, isLoading: statusIsLoading } = useMutation(statusCourse, {
		onSuccess: async () => {
			SuccessToastMessage({
				title: "Status Changed Successfully",
				id: "status_course_success",
			});
			fetchCourseList();
		},
		onError: async (e: HTTPError) => {
			// const error = await e.response.text();
			// console.log("error", error);
			ErrorToastMessage({ error: e, id: "status_course" });
		},
	});
		...
		<div className="">
											...
											return (
												<React.Fragment
													...
													<tr
														...
														onClick={() => { }}>
														<TableData data={item.course_name} />
														<TableData data={item.course_shortcode} />
														<TableData
														...
														<TableData data={
															<>
																{isEditAccess ? (
																<Link to={'/admin/course-details/' + item.id}>
																	<Icon icon="pencil-square-outline" className="w-6 h-6 cursor-pointer"
																		data-tooltip-id="tooltip" data-tooltip-content="Edit Course" />
																</Link>
																) : ("") }
																{isDeleteAccess ? (
																<Icon icon="trash-outline" className="w-6 h-6 cursor-pointer"
																		data-tooltip-id="tooltip" data-tooltip-content="Delete Course" onClick={() => showDeleteModal(item.id)} />
																) : ("") }
															</>
														} />
													</tr>
			...
			<div>
				<PaginationBar
					...
					totalCampaigns={
						courseList?.total_records ? courseList?.total_records : 0
					}
