...
import { useIndustrys,deleteIndustry,statusIndustry } from "api/services/industryService";
import { ErrorToastMessage, SuccessToastMessage } from "api/services/user";
import { useMutation } from "react-query";
import { HTTPError } from "ky";
import PaginationBar from "components/ui/paginationBar";
import TableHead from "components/ui/table/tableHead";
import { IIndustry, TableHeadType,ConfirmPopupDataType } from "utils/datatypes";
...
function Industries() {
	...
	const ConfirmPopupData : ConfirmPopupDataType =
		{ title: "Industry Delete", text: "Are you sure you want to delete Industry?" };

	const {
		isLoading,
		data: industryList,
		refetch: fetchIndustryList,
		isFetching: isFetchingIndustryList,
	} = useIndustrys({
		enabled: true,
		...
		const fetchData = async () => {
		  ...
		  const checkIfIncludes = (value: number) => {
			...
			return rolepermid.includes(value);
			  ...
			  .get(
				  `api/v1/submenu/action=Add&module_alias=industrylist`
			  );
			...
			const editmodule = await apiClient
			  .get(
				  `api/v1/submenu/action=Edit&module_alias=industrylist`
			  );
			  ...
			  .get(
				  `api/v1/submenu/action=Delete&module_alias=industrylist`
			  );
	...
	const { mutate: deleteItem, isLoading: uploadIsLoading } = useMutation(deleteIndustry, {
		onSuccess: async () => {
			SuccessToastMessage({
				title: "Delete Industry Successfully",
				id: "delete_industry_success",
			});
			fetchIndustryList();
		},
		onError: async (e: HTTPError) => {
			...
			ErrorToastMessage({ error: e, id: "delete_industry" });
		},
	});
	...
	const { mutate: statusItem, isLoading: statusIsLoading } = useMutation(statusIndustry, {
		onSuccess: async () => {
			SuccessToastMessage({
				title: "Status Changed Successfully",
				id: "status_industry_success",
			});
			fetchIndustryList();
		},
		onError: async (e: HTTPError) => {
			// const error = await e.response.text();
			// console.log("error", error);
			ErrorToastMessage({ error: e, id: "status_industry" });
		},
	});
		...
		<div className="">
			...
			<div className="flex flex-col">
				<table className="w-full border-separate border-spacing-y-2 text-left">
					...
					<tbody className="">
						{isLoading || isFetchingIndustryList ? (
											...
											return (
												<React.Fragment
													...
													<tr
														...
														onClick={() => { }}>
														<TableData data={item.industry_name} />
														<TableData
														...
														<TableData data={
															<>
																{isEditAccess ? (
																<Link to={'/admin/industry-details/' + item.id}>
																	<Icon icon="pencil-square-outline" className="w-6 h-6 cursor-pointer"
																		data-tooltip-id="tooltip" data-tooltip-content="Edit Industry" />
																</Link>
																) : ("") }
																{isDeleteAccess ? (
																<Icon icon="trash-outline" className="w-6 h-6 cursor-pointer"
																		data-tooltip-id="tooltip" data-tooltip-content="Delete Industry" onClick={() => showDeleteModal(item.id)} />
																) : ("") }
															</>
														} />
													</tr>
			...
			<div>
				<PaginationBar
					...
					totalCampaigns={
						industryList?.total_records ? industryList?.total_records : 0
					}
