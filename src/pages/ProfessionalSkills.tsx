...
import { useProfessionalskills, deleteProfessionalskill, statusProfessionalskill} from "api/services/professionalskillService";
import { ErrorToastMessage, SuccessToastMessage } from "api/services/user";
import { useMutation } from "react-query";
import { HTTPError } from "ky";
import PaginationBar from "components/ui/paginationBar";
import TableHead from "components/ui/table/tableHead";
import { IProfessionalskill, TableHeadType,ConfirmPopupDataType } from "utils/datatypes";
...
function ProfessionalSkills() {
	...
	const ConfirmPopupData : ConfirmPopupDataType =
		{ title: "Professionalskill Delete", text: "Are you sure you want to delete Professionalskill?" };

	const {
		isLoading,
		data: professionalskillList,
		refetch: fetchProfessionalskillList,
		isFetching: isFetchingProfessionalskillList,
	...
	console.log("professionalskillList", professionalskillList);
		...
		const fetchData = async () => {
		  ...
		  const checkIfIncludes = (value: number) => {
			...
			return rolepermid.includes(value);
			  ...
			  .get(
				  `api/v1/submenu/action=Add&module_alias=professionalskilllist`
			  );
			...
			const editmodule = await apiClient
			  .get(
				  `api/v1/submenu/action=Edit&module_alias=professionalskilllist`
			  );
			  ...
			  .get(
				  `api/v1/submenu/action=Delete&module_alias=professionalskilllist`
			  );
	...
	const { mutate: deleteItem, isLoading: uploadIsLoading } = useMutation(deleteProfessionalskill, {
		onSuccess: async () => {
			SuccessToastMessage({
				title: "Delete Professionalskill Successfully",
				id: "delete_professionalskill_success",
			});
			fetchProfessionalskillList();
		},
		onError: async (e: HTTPError) => {
			...
			ErrorToastMessage({ error: e, id: "delete_professionalskill" });
		},
	});
	...
	const { mutate: statusItem, isLoading: statusIsLoading } = useMutation(statusProfessionalskill, {
		onSuccess: async () => {
			SuccessToastMessage({
				title: "Status Changed Successfully",
				id: "status_professionalskill_success",
			});
			fetchProfessionalskillList();
		},
		onError: async (e: HTTPError) => {
			// const error = await e.response.text();
			// console.log("error", error);
			ErrorToastMessage({ error: e, id: "status_professionalskill" });
		},
	});
	...
	return (
		<div className="">
			<div className="flex justify-between border-b border-border pb-5 items-center">
				...
				<Link
					to={"/admin/professionalskill-details"}
			...
			<div className="flex flex-col">
				<table className="w-full border-separate border-spacing-y-2 text-left">
					...
					<tbody className="">
						{isLoading || isFetchingProfessionalskillList ? (
							...
							<>
								{professionalskillList &&
									professionalskillList?.data &&
									professionalskillList?.data?.length ? (
									professionalskillList?.data?.map(
										(
											...
											return (
												<React.Fragment
													...
													<tr
														...
														onClick={() => { }}>
														<TableData data={item.skill_name} />
														<TableData
														...
														<TableData data={
															<>
																{isEditAccess ? (
																<Link to={'/admin/professionalskill-details/' + item.id}>
																	<Icon icon="pencil-square-outline" className="w-6 h-6 cursor-pointer"
																		data-tooltip-id="tooltip" data-tooltip-content="Edit Professionalskill" />
																</Link>
																) : ("") }
																{isDeleteAccess ? (
																<Icon icon="trash-outline" className="w-6 h-6 cursor-pointer"
																		data-tooltip-id="tooltip" data-tooltip-content="Delete Professionalskill" onClick={() => showDeleteModal(item.id)} />
																) : ("") }
															</>
														} />
													</tr>
										...
										<td
											...
											<NotFound
												...
												setOpenCreate={(
													...
													if (value)
														navigate(
															"/admin/professionalskill-details",
														);
												}}
											/>
										</td>
			...
			<div>
				<PaginationBar
					...
					totalCampaigns={
						professionalskillList?.total_records ? professionalskillList?.total_records : 0
					}
