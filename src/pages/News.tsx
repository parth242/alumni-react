...
import { useNewss,deleteNews } from "api/services/newsService";
import { HTTPError } from "ky";
import PaginationBar from "components/ui/paginationBar";
import TableHead from "components/ui/table/tableHead";
import { IUser, TableHeadType,ConfirmPopupDataType,INews } from "utils/datatypes";
...
function News() {
	...
	const ConfirmPopupData : ConfirmPopupDataType =
		{ title: "News Delete", text: "Are you sure you want to delete News?" };
		...
		const fetchData = async () => {
		  ...
		  const checkIfIncludes = (value: any) => {
			...
			return rolepermid.includes(value);
			  ...
			  .get(
				  `api/v1/submenu/action=Add&module_alias=newslist`
			  );
			...
			const editmodule = await apiClient
			  .get(
				  `api/v1/submenu/action=Edit&module_alias=newslist`
			  );
			  ...
			  .get(
				  `api/v1/submenu/action=Delete&module_alias=newslist`
			  );
	...
	const handlePageSizeChange = (
		news: React.ChangeEvent<HTMLSelectElement>,
	) => {
		const { value } = news.target;
	...
	const { mutate: deleteItem, isLoading: uploadIsLoading } = useMutation(deleteNews, {
		onSuccess: async () => {
			SuccessToastMessage({
				title: "Delete News Successfully",
				id: "delete_news_success",
			});
			fetchNewsList();
		},
		onError: async (e: HTTPError) => {
			...
			ErrorToastMessage({ error: e, id: "delete_news" });
		},
	});
		...
		<div className="">
												...
												<React.Fragment
													...
													<tr
														...
														<TableData data={
															<>
																{isEditAccess ? (
																<Link to={'/admin/news-details/' + item.id}>
			...
			<div>
				<PaginationBar
					...
					totalCampaigns={
						newsList?.total_records ? newsList?.total_records : 0
					}
